...
SELECT AVG(balance) avg_balance,percentile(age, 0.5) median_age FROM bankmarket;


-- 6.	Check if age matters in marketing subscription for deposit
SELECT age,SUM(IF(y='yes',1,0)) NoOfDeposits FROM bankmarket GROUP BY age ORDER BY NoOfDeposits DESC;
